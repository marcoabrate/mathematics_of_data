% FUNCTION: fout = FISTA_fxR(fx, gx, gradf, proxg, x0, Lips, maxit, tolx, F_star, res, verbose)
% PURPOSE:  FISTA (for problem 4 equation 18) with fixed restart algorithm.
%
function iout = FISTA_fxR(fx, gx, gradf, proxg, x0, Lips, maxit, tolx, F_star, res, verbose)
    ...
    semilogy(fg_pl);
    grid on;
    title(sprintf("FISTA with restart every %d iterations", res));
    xlabel("iteration k");
