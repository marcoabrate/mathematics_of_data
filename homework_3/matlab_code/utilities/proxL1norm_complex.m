...
%
function z_out_real = proxL1norm_complex(zc, gamma)
    ...
    m = length(zc)/2;
    z_complex = zc(1:m) + 1i*zc(m+1:2*m);
    
    zout_complex = sign(z_complex).*max(abs(z_complex) - gamma, 0.0); 
    
    z_out_real = [real(zout_complex);imag(zout_complex)];
    
    
    % Perform the soft-thresholding operator.
   % zout  = sign(zc).*max(abs(zc) - gamma, 0.0); 
    
   % zout  = sign(real(zc)).*max(abs(real(zc)) - gamma, 0.0) + 1i*(   sign(imag(zc)).*max(abs(imag(zc)) - gamma, 0.0)     );
