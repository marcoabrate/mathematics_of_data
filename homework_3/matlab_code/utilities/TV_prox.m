function [f, err, px, py] = TV_prox(g, varargin)


% [f,px,py] = TV_prox(g, varargin)
% Proximity operator for the anisotropic TV regularizer 
%
...
while cont 
    ...
    err(k) = sum((-upx(:)+tmp(:).*px(:)).^2 + (-upy(:)+tmp(:).*py(:)).^2)^0.5;
    px = (px + tau * upx)./(1 + tau * tmp);
    py = (py + tau * upy)./(1 + tau * tmp);
    cont = ((k<MaxIter) & (err(k)>tol));
end
if verbose
   fprintf(' \n\n k TV = %g,    \n\n', k);
   fprintf(' \n\n err TV = %g,    \n\n', err(k));
